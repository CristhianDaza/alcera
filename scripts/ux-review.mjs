...
try {
  for (const width of [1440, 768, 390, 320]) {
    ...
    await expect(
      page.getByRole("button", { name: "Añadir otra unidad", exact: false }),
    ).toBeVisible();
    ...
    await page
      .getByRole("link", { name: "Volver a la colección", exact: false })
      .click();
    await expect(
      page.getByRole("combobox", { name: "Categoría", exact: true }),
    ).toHaveValue("Hombre");
    ...
    await expect(
      page.getByRole("heading", { name: "No encontramos esa combinación." }),
    ).toBeVisible();
    await page.getByRole("button", { name: "Ver toda la colección" }).click();
    await expect(page.locator(".product-card")).toHaveCount(products.length);
    await page
      .getByRole("combobox", { name: "Ordenar", exact: true })
      .selectOption("desc");
    ...
    await page.close();
    console.log(
      `PASS ${width}px: layout, search, filters, return navigation, cart, quantity, persistence, WhatsApp link, theme`,
    );
  }
} finally {
  await browser.close();
