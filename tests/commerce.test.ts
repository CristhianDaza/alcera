...
describe("Pedido por WhatsApp", () => {
  it("actualiza el precio desde el catálogo sin cambiar cantidades", () => {
    const result = reconcileCart([line], demoProducts);
    expect(result[0]?.price).toBe(v.price);
    expect(result[0]?.quantity).toBe(2);
  });
  it("bloquea borradores y productos eliminados", () => {
    expect(reconcileCart([line], [])[0]?.available).toBe(false);
    expect(
      reconcileCart([line], [{ ...p, status: "draft" }])[0]?.available,
    ).toBe(false);
  });
  it("detecta variantes agotadas y retiradas", () => {
    expect(reconcileCart([line], [{ ...p, variants: [] }])[0]?.available).toBe(
      false,
    );
    expect(
      reconcileCart(
  ...
  it("incluye cantidad, subtotal y condiciones en el mensaje", () => {
    const message = whatsappMessage([{ ...line, price: 100000 }], "Esencia");
...
describe("Validación administrativa", () => {
  ...
  it("rechaza precios inválidos, slugs inseguros y variantes duplicadas", () => {
    for (const change of [
      ...
      expect(productSchema.safeParse({ ...valid, ...change }).success).toBe(
        false,
      );
  });
