...
beforeEach(() => {
  ...
  vi.stubGlobal("useRuntimeConfig", () => ({
    ...
    firebaseClientEmail: "test",
    firebasePrivateKey: "test",
  }));
  vi.stubGlobal("createError", (data: object) =>
    Object.assign(new Error("denied"), data),
  );
  verify.mockReset();
});
describe("Permisos de administración", () => {
  ...
  it("rechaza tokens inválidos o revocados", async () => {
    verify.mockRejectedValue(new Error("invalid"));
    await expect(requireAdmin({} as never)).rejects.toMatchObject({
      statusCode: 401,
    });
  });
  it("rechaza cuentas autenticadas sin atributo admin", async () => {
    verify.mockResolvedValue({ uid: "user" });
    await expect(requireAdmin({} as never)).rejects.toMatchObject({
      statusCode: 403,
    });
  });
  it("comprueba revocación y permite administradores", async () => {
    verify.mockResolvedValue({ uid: "admin", admin: true });
